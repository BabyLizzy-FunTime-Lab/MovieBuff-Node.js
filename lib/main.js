...
function top5_URL_array(res) {
  ...
  switch (response_obj.Response) {
    case "True":
      ...
      var array_ID_URL = [];
      var response_length = response_obj.Search.length;

      if (response_length > 5) {
        for (var i = 0; i < 5; i++) {
          var imdbID_ID = response_obj.Search[i].imdbID;
          var IDsearch_URL = createURL(imdbID_ID, "ID");
          array_ID_URL.push(IDsearch_URL);
        }
      } else {
        for (var i = 0; i < response_length; i++) {
          var _imdbID_ID = response_obj.Search[i].imdbID;

          var _IDsearch_URL = createURL(_imdbID_ID, "ID");

          array_ID_URL.push(_IDsearch_URL);
        }
      }
...
function search_elements() {
  var poster = elementMaker("img", false, "poster");
  var title = elementMaker("h3", false, "title");
  var year = elementMaker("h4", false, "year");
  var genre = elementMaker("div", false, "genre");
  var type = elementMaker("div", false, "type");
  var director = elementMaker("div", false, "director");
  var actors = elementMaker("div", false, "actors");
  var awards = elementMaker("div", false, "awards");
  var plot = elementMaker("div", false, "plot");
  var info_container = elementMaker("div", false, "info_container_search");
  var search_element_array = new Array(poster, title, year, genre, type, director, actors, awards, plot, info_container);
...
function availability_check(movie_data, poster) {
  if (poster) {
    ...
    if (movie_data === "N/A") {
      var sorryposter = "../images/sorry.gif";
      return sorryposter;
    } else {
      return movie_data;
    }
...
function render_search(topresults_array) {
  ...
  var target_container = elementID("searchresults");
  var movie_containers = new Array();
  var movie_elements = new Array();

  for (var i = 0; i < topresults_array.length; i++) {
    var container = elementMaker("section", false, "movie_container");
  ...
  topresults_array.forEach(function (object, resultsindex) {
    var moviedata = Object.entries(object);
  ...
  movie_elements.forEach(function (elementarray, movie) {
    var movie_info_container = elementarray[9];
...
function run_getRequest(requestURL) {
  getRequest_Promise(requestURL).then(function (values) {
    // console.log(values);
    var ID_URLarray = top5_URL_array(values);
    Promise.all( // Make get request for every ID URL.
    ID_URLarray.map(function (ID_URL) {
      return getRequest_Promise(ID_URL);
    })).then(function (values) {
      // console.log(values);
      var movie_data_array = new Array();
      ...
      elementID("searchresults").style.display = "block"; // Call rendering function
...
function generate_featuredMovies_elements() {
  ...
  var awards = elementMaker("div", false, "awards");
  var plot = elementMaker("div", false, "plot");
  var info_container = elementMaker("div", false, "info_container_featured");
  var element_array = new Array(poster, title, year, awards, plot, info_container);
  return element_array;
}

function render_featuredMovies_Data(movie_array) {
  var target_container = elementID("featuredmovies");
  var movie_containers = new Array();
  var movie_elements = new Array();
  movie_array.forEach(function (movie, index) {
    ...
    var info_container = movie_elements[index][5];
...
function featuredMovies(movie1_ID, movie2_ID) {
  var search_URL_1 = createURL(movie1_ID, "ID");
  var search_URL_2 = createURL(movie2_ID, "ID");
  Promise.all([getRequest_Promise(search_URL_1), getRequest_Promise(search_URL_2)]).then(function (values) {
    var movie_1 = JSON.parse(values[0]);
    var movie_2 = JSON.parse(values[1]);
    var movie_array = new Array(movie_1, movie_2);
    return movie_array;
  }).then(function (movie_array) {
...
window.onload = featuredMovies("tt1343727", "tt0103064"); // EVENTHANDELING click & enter

elementID("search_btn").addEventListener("click", function () {
  var requestURL = createURL(elementValue("search_input"), "string");
  run_getRequest(requestURL);
});
elementID("search_input").addEventListener("keydown", function (event) {
  if (event.keyCode === 13) {
    event.preventDefault();
    var requestURL = createURL(this.value, "string");
    run_getRequest(requestURL);
  }
